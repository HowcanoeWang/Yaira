function [linear,results] = ndgridcalculate1( data_calcu,sample_size,sample_accuracy )
...
if size(X,4)>1
    ...
    linear=interpn(Y,X,Z,T,data_calcu{6},YI,XI,ZI,TI,'linear');
    if isempty(find(isnan(data_calcu{6})==1, 1))
        results=['└' data_calcu{1} ' 成功计算'];
    else
        results=['└' data_calcu{1} ' 其他插值成功，spline插值数据量不足'];
    end
else
    [YI,XI,ZI]=ndgrid(y0:(y1-y0)/sample_accuracy:y1,x0:(x1-x0)/sample_accuracy:x1,z0:(z1-z0)/sample_accuracy:z1);
    linear=interpn(Y,X,Z,data_calcu{6},YI,XI,ZI,'linear');
    results=['└' data_calcu{1} ' 成功计算(单一时间)'];
end
