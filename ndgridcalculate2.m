function [nearest] = ndgridcalculate2( data_calcu,sample_size,sample_accuracy )
...
if size(X,4)>1
    ...
    nearest=interpn(Y,X,Z,T,data_calcu{6},YI,XI,ZI,TI,'nearest');
else
    [YI,XI,ZI]=ndgrid(y0:(y1-y0)/sample_accuracy:y1,x0:(x1-x0)/sample_accuracy:x1,z0:(z1-z0)/sample_accuracy:z1);
    nearest=interpn(Y,X,Z,data_calcu{6},YI,XI,ZI,'nearest');
end
